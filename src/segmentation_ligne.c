    ...
    for(int i =0;i<image->w;i++)
    {
        ...
        if(black && !line)
        {
            ...
            for(i=0;i<image->w;i++)
            {
                ...
                if(pos>=0)
                {
                    putpixel(image,i,pos,SDL_MapRGB(fmt,255,105,180));
                }
            }
        }
        if(!black && line)
        {
            ...
            for(i=0;x<image->w;i++)
            {
                ...
                if(pos<image->h)
                {
                    putpixel(image,i,j,SDL_MapRGB(fmt,255,105,180));
                }
            }
            segmentation_columns(image,begin,end);
        }
        black=0;
    }
}
...
end_line,SDL_PixelFormat *fmt)
{
    ...
    for(int x = 0; x <image-> w; x++)// through the columns
    {
        ...
        if(end_c==1 && black==1)// color the previous column of the caracter
        {
            ...
            for(int y = begin_line; y < end_line; y++)
            {
                if(x - 1 > 0)//previous column
                {
                    ...
                    putpixel(image, x - 1, y, SDL_MapRGB(fmt, 1,
                    149,255);
        ...
        if(end_c==0 && end_black==1)// color the next column of the caracter
        {
            ...
            for(int y = begin_line; y < end_line; y++)
            {
                putpixel(image, x, y,  SDL_MapRGB(fmt, 1, 149, 255);
            }
        }
    }
}
